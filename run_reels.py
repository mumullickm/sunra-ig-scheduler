"""Sunra reels auto-publisher: IG Reels + FB Reels. Self-healing. Idle until META_PAGE_TOKEN."""
...
def publish_fb_reel(page_id, reel):
    ...
    req = urllib.request.Request(up, data=b"", method="POST",
                                 headers={"Authorization": f"OAuth {TOKEN}", "file_url": video_url(reel)})
    try:
    ...
    fin = _post(f"{page_id}/video_reels", {"upload_phase": "finish", "video_id": vid,
                                           "video_state": "PUBLISHED", "description": CAPS[reel]})
