"""Sunra statics auto-publisher: IG feed + FB photo. Posts anything due-but-unposted
(self-heals late crons). Idle until META_PAGE_TOKEN secret is set. Publisher derives Page+IG
from the token."""
...
def _post(path, params, timeout=180):
    ...
    req = urllib.request.Request(f"{GRAPH}/{path}", data=urllib.parse.urlencode(p).encode(), method="POST")
    try:
...
def ig_user_id():
    if CFG.get("igUserId"):
        return CFG["igUserId"]
    return _get("me", {"fields": "instagram_business_account"}).get("instagram_business_account", {}).get("id")
...
def publish_fb_photo(page_id, sid):
    r = _post(f"{page_id}/photos", {"url": image_url(sid), "message": CAPS[sid], "published": "true"})
...
def main():
    ...
    if not TOKEN:
        print(f"[{stamp}] META_PAGE_TOKEN not set — scheduler idle."); return
